  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 overflow-y-auto h-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              {period === "daily" ? "За 30 дней" : period === "weekly" ? "За 12 недель" : "За 12 месяцев"} — Сообщения
            </CardTitle>
        ...
        <Card className="shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Новые лиды</CardTitle>
            <Users className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{timeSeries?.totalLeads ?? 0}</div>
            <p className="text-xs text-muted-foreground">Всего новых клиентов</p>
          </CardContent>
        </Card>

        <Card className="shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Брони</CardTitle>
            <CheckSquare className="h-4 w-4 text-green-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{timeSeries?.totalBookings ?? 0}</div>
            <p className="text-xs text-muted-foreground">Подтверждённые брони</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="shadow-sm">
        ...
        <CardContent>
          {tsLoading ? (
            <div className="h-64 flex items-center justify-center animate-pulse text-muted-foreground">Загрузка...</div>
          ) : chartData.length === 0 ? (
          ...
          {tsLoading ? (
            ...
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <Bar dataKey="leads" name="Новые лиды" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} opacity={0.8} />
                <Bar dataKey="bookings" name="Брони" fill="#22c55e" radius={[4, 4, 0, 0]} opacity={0.8} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </CardContent>
      ...
      {dashboard && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Всего диалогов", value: dashboard.totalConversations, sub: `${dashboard.activeConversations} активных` },
            { label: "Всего лидов", value: dashboard.totalLeads, sub: `${dashboard.hotLeads} горячих` },
            { label: "Брони", value: dashboard.bookedLeads, sub: "подтверждено" },
            { label: "Сообщений сегодня", value: dashboard.todayMessages, sub: "по всем каналам" },
          ].map((stat) => (
